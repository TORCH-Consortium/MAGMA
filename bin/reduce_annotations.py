...
def extract_reduce_write_annotations(input_filename, output_filename):
    ...
    with open(input_filename) as input_annotations_file:
        ...
        annotations_order_string = list(
            filter(lambda a_line: a_line.find(annotations_order_marker) != -1, lines)
        )[0]
        annotations_order_list = (
            annotations_order_string.strip()
            .split(":")[-1]
            .replace("[", "")
            .replace("]", "")
            .split(",")
        ...
        eliminated_annotation_string = (
            "-an " + " -an ".join(annotations_order_list[:-1]).strip()
        )
        print("ANNOTATIONS_ANALYSIS: Dropping : " + annotations_order_list[-1])
        print(
            "ANNOTATIONS_ANALYSIS: Final annotation string : ",
            eliminated_annotation_string,
        )
...
def tranches_and_annotations(
    input_annotations_file, input_tranches_filename, output_tranches_file
):
    ...
    with open(input_tranches_filename) as input_tranches_file:
        ...
        print(
            "TRANCHES_ANALYSIS: minVQSLod score (for initial ordered annotations list) corresponding to targetTruthSensitivity: 99.90 is => ",
            minVQSLod_score,
        )
...
def main(args=None):
    ...
    extract_reduce_write_annotations(
        args.input_annotations_file, args.output_annotations_file
    )

    tranches_and_annotations(
        args.input_annotations_file,
